...
function editManager(e) {
    e.preventDefault();
    $('[name="idxEditExtraManager"]').remove();
    var id = $(this).attr('data-edit');
    var form = document.querySelector('[data-hook="registration-form"]');
    $(form).append($('<input type="hidden" name="idxEditExtraManager" value='+id+' />'));
}
...
$('body').on('click', '[data-edit]', editManager);
...
var rulesFileUpload = {
    ...
    'Registration[ContactPerson][<?= $contactPersonIdx; ?>][otherEmployment]': {
        required: {
            depends: function(element) {
                var needsValue = parseInt($('[data-hook="toggle-employment"]').find(":selected").val());
                if (needsValue) {
                    return true;
                } else {
                    return false;
